...
source = `${source.slice(0, forumStart)}${forumComponent}${source.slice(forumEnd)}`;
...
source = replaceExact(
  ...
  `${orientationComponent}function eh({analyst:e,secondCycle:t,onSearch:n,onClose:a})`,
  1,
...
.orientation-body small{display:block;margin-top:10px;color:#555}
...
.forum-water-event{position:absolute;z-index:35;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;color:#ffd7d7;background:linear-gradient(to bottom,rgba(16,8,42,.28) 0 42%,rgba(78,0,12,.94) 42% 100%);pointer-events:none;animation:forumSubmerge .85s steps(3,end)}
.forum-water-event:after{content:"AI 交流论坛 / RINGBOARD 2.04";position:absolute;left:8%;right:8%;top:58%;padding:18px;color:#ff9ba1;border-top:2px solid #ff4f5b;text-align:center;transform:scaleY(-1);opacity:.72;font:700 clamp(19px,4vw,34px) Georgia,serif;letter-spacing:.08em}
...
.forum-waterline{position:absolute;left:0;right:0;top:42%;height:5px;background:#ff6972;box-shadow:0 -5px 18px #493bad,0 7px 25px #ff1f31}
.forum-reflecting .forum-layout,.forum-reflecting>header,.forum-reflecting .forum-tools{filter:sepia(.7) saturate(1.8) hue-rotate(300deg);transform:translateY(-2px)}
@keyframes forumSubmerge{0%{opacity:0;transform:translateY(3px)}18%{opacity:.92}68%{opacity:1;transform:translateY(0)}100%{opacity:0}}
...
@media (prefers-reduced-motion:reduce){.forum-water-event{animation:none}.forum-reflecting .forum-layout,.forum-reflecting>header,.forum-reflecting .forum-tools{transform:none}}
`;
